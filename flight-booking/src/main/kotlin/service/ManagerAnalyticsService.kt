...
class ManagerAnalyticsService {
    fun getAnalytics(): ManagerAnalyticsResponse {
        ...
        val upcomingFlights = transaction {
            Flights.selectAll().where { Flights.date greaterEq LocalDate.now().toString() }.count().toInt()
        }

        val openTickets = transaction {
            SupportTickets.selectAll().where { (SupportTickets.status eq TicketStatus.OPEN) or (SupportTickets.status eq TicketStatus.IN_PROGRESS) }.count().toInt()
        }
        ...
        val bookingsPerHour = bookingTimeCounts.mapIndexed { hour, count ->
            HourlyBookingCount(
                hour = hour,
                count = count
            )
        ...
        val bookingsPerFlight = allFlights.map {flight ->
        ...
        val bookingsPerRoute = allRoutes.map {route ->
        ...
        val managerAnalyticsResponse = ManagerAnalyticsResponse(
